...
public isolated class Converter {
    ...
    // public isolated function fromBytes(byte[] bytes, string? targetRecordName = (), Encoding encoding = ASCII,  typedesc<record {}> t = <>) returns t|Error = @java:Method {
    //     'class: "io.ballerina.lib.copybook.runtime.converter.Utils"
    // } external;

    public isolated function fromBytes(byte[] bytes, string? targetRecordName = (), Encoding encoding = ASCII) returns map<json>|Error {
        lock {
            check self.validateTargetRecordName(targetRecordName);
            CopybookBytesReader copybookReader = new (bytes.cloneReadOnly(), self.schema, targetRecordName);
            self.schema.accept(copybookReader);
            DataCoercer dataCoercer = new (self.schema, targetRecordName);
            return dataCoercer.coerce(copybookReader.getValue()).clone();
        }
